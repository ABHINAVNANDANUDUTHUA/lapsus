...
const BACKEND_URL = 'https://lapsus-vzwm.vercel.app';
...
function App() {
        ...
        try {
            ...
            if (error.response) {
                ...
                alert(
                    ...
                    `${BACKEND_URL}/predict. Check network / CORS / deployment.`
                );
            } else {
    ...
    // Trigger prediction when slider moves (if a marker exists and sim mode is ON)
    useEffect(() => {
        if (marker && simMode) {
    ...
    }, [rainValue, simMode, marker]); // include marker

    // Helper to safely format FoS, falling back to 'N/A'
    const safeFoS =
        result?.prediction?.details?.FoS !== undefined &&
        result?.prediction?.details?.FoS !== null
    ...
    // Helper to safely access prediction detail properties
    const safeDetail = (key, suffix = '', fallback = 'N/A') =>
        result?.prediction?.details?.[key] !== undefined &&
        result.prediction.details[key] !== null
            ? `${result.prediction.details[key]}${suffix}`
            : fallback;
            ...
            <div className="absolute top-4 left-4 z-[1000] w-80 md:w-96 flex flex-col gap-4">
                ...
                <div className="bg-white/95 backdrop-blur-md p-5 rounded-2xl shadow-xl border-l-8 border-cyan-500">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                        Real-time Landslide Analysiser
                    </p>
                </div>
                    ...
                    {simMode ? (
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-1 text-center">
                                Drag slider to test extreme weather scenarios
                            </p>
                        </div>
                    ) : (
                        <p className="text-xs text-slate-400 italic">
                            Using Live Open-Meteo Weather Data
                        </p>
                    )}
                </div>
                        ...
                        <div className="p-5 space-y-4 max-h-[50vh] overflow-y-auto">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <StatBox
                                        label="Slope Angle"
                                        value={safeData('slope', '°')}
                                    />
                                    <StatBox
                                        label="Elevation"
                                        value={
                                            result?.data?.elevation !== undefined &&
                                            result?.data?.elevation !== null
                                                ? `${Math.round(
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {/* Use EFFECTIVE values from backend */}
                                    <StatBox
                                        label="Cohesion (c)"
                                        value={safeDetail(
                                            'cohesion_effective',
